...
public class DashProjCreateCmd implements Icommand {
	...
	public String method(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		try {
			System.out.println("startDate : " + req.getParameter("task_startdate"));
			System.out.println("endDate : " + req.getParameter("task_enddate"));
			Date startDate = dateFormat.parse(req.getParameter("task_startdate"));
			Date endDate = dateFormat.parse(req.getParameter("task_enddate"));
		...
		try {
			...
			stmt.setDate(2, (java.sql.Date) dto.getTask_startdate());
			stmt.setDate(3, (java.sql.Date) dto.getTask_enddate());
		...
		return "/dashProj?fn=taskRead&proj_pk_num=" + proj_pk_num;
	}
}