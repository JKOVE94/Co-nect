	...
	public void setPost(HttpServletRequest req, HttpServletResponse resp) {
		...
		try {
			...
			String sql = "INSERT INTO project(proj_name, proj_desc,"+
					...
					"proj_fk_user_num, proj_fk_dpart_num, proj_fk_comp_num, proj_created, proj_updated, proj_startdate, proj_enddate) VALUES(?,?,?,?,?,?,?,?,?,?,now(),now(),?,?)";
			...
			stmt.setString(11, format.format(dto.getProj_startdate()));
			stmt.setString(12, format.format(dto.getProj_enddate()));
